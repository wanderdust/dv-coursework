...
let filterTree = (data, context, type="overall") => {
...
// Returns hierarchical data for bubbles
let filterTreeManager = (data, context, type="overall") => {
    let newData = data.map(d => {
        let region = context.filter(e => {
            return e.UKPRN == d["Institution code (UKPRN)"] 
        })
...
let filterUnis = (data, context) => {
    ...
    let addValues = nest.map(d => {
        return {
            ...
            uoas: {
                4: d.values.map(e => e["overall"]["4*"]).reduce((x,y) => x+y) / d.values.length,
                3: d.values.map(e => e["overall"]["3*"]).reduce((x,y) => x+y) / d.values.length,
                2: d.values.map(e => e["overall"]["2*"]).reduce((x,y) => x+y) / d.values.length,
                1: d.values.map(e => e["overall"]["1*"]).reduce((x,y) => x+y) / d.values.length,
                unclassified: d.values.map(e => e["overall"]["unclassified"]).reduce((x,y) => x+y) / d.values.length
            }
        }
    })
    return addValues
